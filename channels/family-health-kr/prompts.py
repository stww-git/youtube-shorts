...
5. 금지: 이모지, 괄호, **쉼표(,)** 등 모든 특수문자 사용 금지.
...
당신은 건강 정보 요약 전문가입니다.
아래 건강 칼럼 원문을 읽고, 시청자가 메모할 만한 **핵심 실천 사항**을 4~5개 추출해주세요.

[원문]
{article_content}

[추출 규칙]
1. **체크리스트 형식**: "1. [짧은 문장]" 형태로 숫자 번호를 붙여 작성
2. **실천 가능한 내용만**: 집에서 바로 할 수 있는 행동/습관
3. **한 줄에 15자 이내**: 짧고 간결하게
4. **의학 용어 금지**: 쉬운 말로 풀어쓰기
5. **4~5개만**: 너무 많으면 안 됨
